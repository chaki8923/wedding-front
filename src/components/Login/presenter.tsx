...
export function Presenter(props: Props) {
  ...
  return (
    <div className="relative w-full h-screen flex flex-col justify-center items-center bg-center  bg-no-repeat" style={{ backgroundImage: "url('/leaf19.png')", height: 'calc(100vh - 2rem)' }}>
      <form className="flex flex-col items-center p-8 max-w-sm w-full mt-8 text-center" onSubmit={props.handleSubmit(props.onSubmit)}>
        ...
        <button disabled={props.isSubmitting} type="submit" className="w-full mt-2 py-3.5 flex justify-center align-center text-white bg-green-600 hover:bg-green-700 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:outline-none inline-flex items-center">
          {props.isSubmitting ?
        ...
        <Link href='/sign_up' className="w-full mt-6 text-xs text-black text-center cursor-pointer underline hover:text-gray-400">
          新規登録はこちら
        </Link>
      </form>
    </div>
